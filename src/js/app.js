...
addTiket.addEventListener("click", () => {
  ...
  submitDescription.addEventListener("click", (e) => {
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4){
        tiketPad.insertAdjacentHTML( // добавляем новый тикет со значениями
        "beforeend",
        `
...
tiketPad.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("delete")) {
    ...
    document.querySelector(".submitDelete").addEventListener("click", (e) => {
      e.preventDefault();
      const xhr = new XMLHttpRequest();
  ...
  // correct
    ...
    document.querySelector(".descriptionName").value =
      tiketCorrectValue.querySelector(".name").textContent;

    // Значение поля Полное содержание (при корректировке данных)
    document.querySelector(".fullDescriptionName").value =
      tiketCorrectValue.querySelector(".name").dataset.fulldescription;
    ...
    submitCorrectDescription.addEventListener("click", () => {
      ...
      tiketCorrectValue.querySelector(".name").dataset.fulldescription =
        ...
        const xhr = new XMLHttpRequest();
        const body = `id=${encodeURIComponent(tiketCorrectValue.dataset.id)}&name=${encodeURIComponent(document.querySelector(".descriptionName").value)}&description=${encodeURIComponent(document.querySelector(".fullDescriptionName").value)}`
        xhr.onreadystatechange = () => {
          if (xhr.readyState !== 4) {
            return;
          }
    ...
    const xhr = new XMLHttpRequest();
    const body = `id=${encodeURIComponent(idTicket)}&status=${encodeURIComponent(conditionCheckBox)}`
    xhr.onreadystatechange = () => {
      if (xhr.readyState !== 4) {
        return;
      }
    ...
    console.log("Клик на тикет");
    // ообработка клика на тикет
    const tiket = e.target.closest(".tiket");

    if (!tiketPad.querySelector(".fullDes")) {
      console.log(tiket.querySelector(".fullDes"));
      tiketFullDescription =
        tiket.querySelector(".name").dataset.fulldescription;
